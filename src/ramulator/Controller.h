        ...
        void tick()
        {
            ...
            if (pending.size())
            {
                ...
                if (req.depart <= clk)
                {
                    ...
                    // ME: pending queue only stores READ req,
                    // ME: which is from core, therefore a callback func is needed
                    req.callback(req);
                    pending.pop_front();
                }
            }
            ...
            {
                channel->update_serving_requests(req->addr_vec.data(), -1, clk);
                req->callback(*req); // ME: this line should be removed when mode dram is implemented
            }
