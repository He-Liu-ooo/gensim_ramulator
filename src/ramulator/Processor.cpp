...
void Core::tick()
{
  ...
  if (req_type == Request::Type::READ)
  {
    ...
    Request req(req_addr, req_type, callback, id); // ME: this id is coreid
    if (!send(req))
      return;
  ...
  {
    if (!reached_limit)
    { // if the length of this trace is shorter than expected length, then record it when the whole trace finishes, and set reached_limit to true.
...
bool Trace::get_filtered_request(long &bubble_cnt, long &req_addr, Request::Type &req_type)
{
  ...
  if (file.eof() || line.size() == 0)
  {
    ...
    { // starting over the input trace file
      getline(file, line);
      line_num++;
    }
  }
